...
/* ───── edit your base URL if needed ───── */
const API = `${APP_CONTANTS.API_FOR_WEBSITE}/getallusers`;
const DELETE_API = `${APP_CONTANTS.API_FOR_WEBSITE}/user`;
...
export default function UserTable() {
  ...
  /* ─── filter whenever searchTerm / district change ─── */
  useEffect(() => {
  ...
  return (
    <div className="p-4 font-poppins mt-7">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-4">
        ...
        >
          <option value="">All Districts</option> {/* fixed from "kurnool" */}
          {AP_DISTRICTS.map((d) => (
        ...
        {/* clear button shows only if either filter active */}
        {(searchTerm || district) && (
          <button
            onClick={() => {
              setSearchTerm("");
              ...
              {users.map((u, idx) => (
                <tr key={u._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 flex gap-2 justify-center">
                    <button
                      onClick={() => {
                         if (!window.confirm(`Edit User ${u.name} ?`)) return;
                         setEditUser(u)
                      }}
                      className="px-2 py-1 cursor-pointer bg-blue-600 text-white text-xs rounded hover:bg-blue-700"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(u._id)}
                      className="px-2 py-1 cursor-pointer bg-red-600 text-white text-xs rounded hover:bg-red-700"
                    >
                      Delete
                    </button>
                  </td>
